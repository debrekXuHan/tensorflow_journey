...
correct_sum = 0
cnt = int(test_num / step)

for i in range(cnt):
    start = i * step
    end = (i + 1) * step
    test_accuracy = sess.run(accuracy, feed_dict = {
        x: mnist.test.images[start:end, :], y_: mnist.test.labels[start:end, :], keep_prob: 1.0})
    correct_sum += test_accuracy * step

test_accuracy = correct_sum / test_num
